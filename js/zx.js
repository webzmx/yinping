...
$(".btn-loadmore").click(function(){
    ...
    x=$(this).index(".btn-loadmore")
        var arr2=["0","1","2","3","4","5","6","7"];
        var endVal=parseFloat(arr2[x])+1*10;
       var info=$(this).siblings(".info-subnav-loadmore");
       $.ajax({
           url:"php/info_pro.php", 
           data:{reid:endVal},          
           dataType: "json",
           success:function(data){
               ...
               for(var i=0;i<data.length;i++){                
                       str+='<li class="clearfix">'
                               + '<div class="subnav-deial-pro">'
                                   +' <img src=""  data-original="images/'+data[i].imgSrc+'" alt="">'
                                +'</div>'
                               +' <div class="subnav-detial-write">'
                                  +  '<h3 class="detial-title"><a href="#">'+data[i].title+'</a></h3>'
                                  + ' <p class="detial-brand">'
                                  ...
                                  + ' </p>'
                                   ...
                                   + '<p class="detial-yinping">'+data[i].yinping+'</p>'
                                   + '<p class="detial-time">'
                                   +'<i class="icon icon-shijianzhongbiao"></i>'
                                    +    ''+data[i].time+''
                                  + ' </p>'
                               + '</div>'
                           +'</li>'                    
           ...
           beforeSend:function(){
            $(".btn-loadmore").eq(x).html('<span class="loading"><span></span><span></span><span></span></span>正在加载中. . .').show();
            
           },
           complete:function(){
            $(".btn-loadmore").eq(x).hide();
